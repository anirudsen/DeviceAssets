...
@app.route('/v1/iSolveDeviceAsset',methods=['POST'])
def getData():
    ...
    if filtercondition == '*'  and incrementaldate != ' ' :
        sql_query = "SELECT * FROM (SELECT *, Row_number() OVER (ORDER BY Device_Asset_Identifier DESC) AS rownum FROM [dbo].iSolve_Device_Asset_Stg)tb1 WHERE rownum between " + offset + "AND " + limit + " AND Last_Update_Date >='"+incrementaldate+"';"
        print(sql_query)
        #sql_query = "SELECT * FROM "+ "dbo.iSolve_Asset_Stg" +" WHERE Last_Update_Date >='" + strDate + "';"
    if filtercondition == '*'  and incrementaldate == ' ' :
        sql_query = "SELECT * FROM (SELECT * , Row_number() OVER (ORDER BY Device_Asset_Identifier DESC) AS rownum FROM [dbo].iSolve_Device_Asset_Stg)tbl WHERE rownum between " + offset + "AND " + limit +";"
    '''
    if filtercondition == '*'  and incrementaldate != ' ' :
